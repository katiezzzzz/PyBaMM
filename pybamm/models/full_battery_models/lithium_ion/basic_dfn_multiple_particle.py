...
class BasicMPMDFN(pybamm.lithium_ion.BaseModel):
    """Doyle-Fuller-Newman (DFN) model of a lithium-ion battery, from [2]_, with
    many particles and resistance
    ...
    **Extends:** :class:`pybamm.lithium_ion.BaseModel`
    ...
    def __init__(self, specs, name="Multi-particle Doyle-Fuller-Newman model"):
        super().__init__({}, name)
        ...
        param = self.param
        self.variables = {}
        neg_particle_specs, pos_particle_specs = specs
        ######################
        # Variables
        ######################
        ...
        # domains
        neg_particles = []
        for i in range(len(neg_particle_specs)):
            c_s_n = pybamm.Variable(
                f"Negative particle concentration {i}",
                domain="negative particle",
                auxiliary_domains={"secondary": "negative electrode"},
            )
            neg_particles.append(c_s_n)
            self.variables.update(
                {
                    f"Negative particle concentration {i}": c_s_n,
                    f"Negative particle surface concentration {i}": pybamm.surf(c_s_n),
                }
            )
        pos_particles = []
        for i in range(len(pos_particle_specs)):
            c_s_p = pybamm.Variable(
                f"Positive particle concentration {i}",
                domain="positive particle",
                auxiliary_domains={"secondary": "positive electrode"},
            )
            pos_particles.append(c_s_p)
            self.variables.update(
                {
                    f"Positive particle concentration {i}": c_s_p,
                    f"Positive particle surface concentration {i}": pybamm.surf(c_s_p),
                }
            )
        ...
        neg_particle_currents = []
        j_n_total = 0
        j_p_total = 0
        for i, c_s_n in enumerate(neg_particles):
            zeta_n = neg_particle_specs[i][1]
            ...
            neg_particle_currents.append(j_n)
            j_n_total += zeta_n * j_n
        pos_particle_currents = []
        for i, c_s_p in enumerate(pos_particles):
            zeta_p = pos_particle_specs[i][1]
            ...
            pos_particle_currents.append(j_p)
            j_p_total += zeta_p * j_p

        j_s = pybamm.PrimaryBroadcast(0, "separator")
        j_total = pybamm.Concatenation(j_n_total, j_s, j_p_total)
        ...
        for i, specs in enumerate(neg_particle_specs):
            R_n = specs[0]
            c_s_n = neg_particles[i]
            j_n = neg_particle_currents[i]
            ...
            self.boundary_conditions[c_s_n] = {
                ...
                "right": (-param.C_n * j_n / param.a_n / param.D_n(c_s_surf_n, T), "Neumann"),
            }
        ...
        for i, specs in enumerate(pos_particle_specs):
            R_p = specs[0]
            c_s_p = pos_particles[i]
            j_p = pos_particle_currents[i]
            N_s_p = -param.D_p(c_s_p, T) * pybamm.grad(c_s_p)
            self.rhs[c_s_p] = -(1 / param.C_p) * pybamm.div(N_s_p)
            self.boundary_conditions[c_s_p] = {
                "left": (pybamm.Scalar(0), "Neumann"),
                "right": (-param.C_p * j_p / param.a_p / param.gamma_p / param.D_p(c_s_surf_p, T), "Neumann"),
            }
        ...
        sigma_eff_p = param.sigma_p * eps_p ** param.b_s_p
        i_s_p = -sigma_eff_p * pybamm.grad(phi_s_p)
        # The `algebraic` dictionary contains differential equations, with the key being
        # the main scalar variable of interest in the equation
        self.algebraic[phi_s_n] = pybamm.div(i_s_n) + j_n_total
        self.algebraic[phi_s_p] = pybamm.div(i_s_p) + j_p_total
        ...
        self.algebraic[phi_e] = pybamm.div(i_e) - j_total
        ...
        self.rhs[c_e] = (1 / eps) * (
            -pybamm.div(N_e) / param.C_e + (1 - param.t_plus(c_e)) * j_total / param.gamma_e
        )
